...
| E — end-to-end render smoke on `ibd_phage_targeting` | live | ✅ committed 2026-05-24 (5 rounds): r1 CLI severity hotpatch + compose; r2 watermark strip + slate-dark + claim_evidence caption autofit; r3 edge-label gap geometry + title acronym fix; r4 gap-threshold 0.4→1.0 + word_wrap off + acronym fix in merge (spec-on-disk = render); r5 edge-label y above node row + claim_evidence with-figure body autofit. Eyeball confirms slides 6/10/13/19 clean; M3 E-3 (slide-13 compound-headline discipline) carried through. Suite 1107 passed. VQA cost across rounds: $2.84. Deferred to Tier F: composer prompt iteration for node-label cap respect (slides 5/9/18 soft-warnings), visual_qa.v1.md prompt iteration to require fontScale-grounded illegible_scale claims (7 false positives in r4 VQA). |
| F — closeout | paperwork | ✅ committed 2026-05-24 (V0_4_ARCHITECTURE §16 M4a → SHIPPED with the 6-tier ship table + 5-round Tier E summary; LAYOUT.md has `visual_qa.py` + `visual_qa.v1.md` + the watermark-strip note on `build_master.py` + the soffice/pdftoppm dep note in §2; DECISIONS.md D-050..D-053; README + HUB_INSTALL note the `--visual-qa` deps; `configure.py` probes for soffice + pdftoppm; auto-memory `project_presentation_maker_v0_4_m4a.md` + MEMORY.md index line) |
