...
| G — live A/B re-run on ibd_phage_targeting | live (~$13) | ✅ ran 2026-05-31 (ibd v0.7 draft_7 rendered, 33 slides, 3.09 MB .pptx with 2 AI images; 100% figure-utilization, 0 relevant_figure_not_used findings, deck_close present, 4 cross_tenant_grounding omissions advisory; ~$13 + in-flight recoveries: extractor punchline-fallback at commit 1d90c63, D-088 judge softening at commit 1000cef, ~$0.50 in image_gen reruns). |
| H — live A/B re-run on functional_dark_matter (and verify resume-cascade healed the v0.6 audit gap retroactively) | live (~$13) | ✅ ran 2026-05-31 (fdm v0.7 draft_7 rendered, 39 slides, 3.87 MB .pptx with 5 AI images; 100% figure-utilization, 0 relevant_figure_not_used, deck_close present, 2 cross_tenant_grounding omissions, 1 data_figure_path_not_in_curated_inventory finding on S1; ~$13). Re-pipeline-rendered including the deck_close splice that Tier C.3 added. The v0.6 fdm audit gap healing test (D-090 resume-cascade against draft_6) was already done at Tier A.2; fdm draft_7 doesn't test it again. |
| I — Adam reads decks + scores metrics + casts veto | review + DECISION | ✅ Adam Tier-I veto 2026-05-31: DON'T SHIP per D-092. Mechanical targets all met (figure-utilization 100% both, deck_close present both, D-088 image-gen widened to claim_evidence with 5 fdm + 2 ibd approvals after Tier-I-fix judge softening). Five qualitative findings: (1) sub-arcs with no curated figures — curator-stage gap the metric doesn't audit; (2) AI images dropped on first run — D-088 judge over-strict, in-session Tier-I-fix relaxed it; (3) fdm slide-32 data_source rendered as on-slide body — visual-QA caught it with high-confidence; (4) v3.2 substory_design silently dropped Conclusion-for-next + Transition-from-prior fields — prompt-layering bug recovered by extractor punchline-fallback; (5) visual-QA findings (11 ibd + 13 fdm warnings) including slide-3 AI-image spoiler pattern (image leaks section-7 statistics on intro slide). All five become v0.8 inputs per Adam-confirmed scoping in V0_8_PUNCH_LIST. |
| J — docs (DECISIONS + RELEASE_NOTES + LAYOUT + SPEC per veto) | docs | ⬛ CANCELLED per veto (D-092 = DON'T SHIP; no release notes / V0_4_ARCH / LAYOUT updates; DECISIONS.md updated with D-092 alone) |
| K — closeout + auto-memory + tag (per veto) | paperwork + tag | ✅ closeout-only per veto: D-092 landed in DECISIONS.md; v0.7 retrospective auto-memory entry at `project_presentation_maker_v0_7.md`; MEMORY.md index updated; V0_8_PUNCH_LIST.md drafted next. No `v0.7.0` tag (per veto) |
