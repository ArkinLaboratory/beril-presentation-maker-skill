...
| A — `substory_design.v3.md` (Q/A/R/C contract) + `slide_compose.v3.md` (register-aware) + `--prompts-version` flag | prompts + orchestrator | ✅ committed 2026-05-25 (new `prompts/substory_design.v3.md` per D-071: substory MUST have **Question:** field (≤25w; missing→cascade P11 missing_question) + **Conclusion for next substory:** (≤25w; missing→missing_conclusion; last-substory exempt); Punchline retained for v1-compat as section_divider title; Q/A/R/C slide-role mapping documented per layout vocabulary; anti-example pairs + self-review checklist updated with v3 fields. New `prompts/slide_compose.v3.md` per D-072: register-discipline preamble (7 forbidden-in-audience-prose patterns with translation table; tool-versions NUANCED per allowlist); post-composition self-check (Pass 1 register, Pass 2 Q/A/R/C role); SUBSTORY_QUESTION + SUBSTORY_CONCLUSION + ALLOWLIST_TERMS added to user-prompt inputs; `compose-fragment.v3` schema_version; all v2 layout/citation/notes behavior unchanged. Orchestrator `--prompts-version {v1,v2,v3}` flag per D-074 with default v2; helpers `_substory_design_prompt_path` + `_slide_compose_prompt_path` dispatch the 3 call sites (stage_substory_design + v0_3 + v0_4 slide_compose paths) by version; v1/v2 both map substory_design → v1 (contract identical); pre-flight prompt-existence loop extended to require v3 files; invalid version → rc=2 with clear error; --help discoverability. 9 new tests cover dispatchers (6 parametrized) + invalid-version validation + help-text + pre-flight-loop literal pin. Suite 1394 passed) |
| A.2 — wire SUBSTORY_QUESTION/CONCLUSION + ALLOWLIST_TERMS through orchestrator | orchestrator + parse_deck_outline | ✅ ready to commit 2026-05-25 (proactive close on the carry surfaced at Tier A: v3 slide_compose.v3.md documents 3 user-prompt inputs (SUBSTORY_QUESTION + SUBSTORY_CONCLUSION + ALLOWLIST_TERMS) that the orchestrator wasn't injecting. Extended `parse_deck_outline.py::_SECTION_FIELDS` with `questions` + `conclusions` entries — D-071 fields parse via the existing `_field_re` bold-label regex (re.escape handles multi-word "Conclusion for next substory" label cleanly). Orchestrator: v0_4 `_compose_one_substory` pre-populates `_M3_BRIEF_QUESTIONS` + `_M3_BRIEF_CONCLUSIONS` + `_M3_ALLOWLIST_TERMS` (loaded via grep+tr from `<project_dir>/references/register_allowlist.md` per D-072) inside `PROMPTS_VERSION=v3` gate; per-substory user-prompt heredoc injects `SUBSTORY_QUESTION=$sub_question` + `SUBSTORY_CONCLUSION=$sub_conclusion` + `ALLOWLIST_TERMS=$_M3_ALLOWLIST_TERMS` when v3. v0_3 `_slide_compose_v0_3` does the same (pre-extract at top of the substory loop; inject per-substory inside the v3 gate). v1/v2 paths NEVER pay the parse-cost + don't bloat the user prompt with unused fields. 10 new tests: 5 for parse_deck_outline (extract per substory + CLI for questions/conclusions + last-substory-empty-string + v1-fixture-backward-compat); 5 for orchestrator wiring (v0_4 injection literal pin + v0_4 gated pre-populate pin + v0_3 path injection pin + parse_deck_outline --help discoverability + allowlist load via grep+tr pin). Suite 1404 passed) |
