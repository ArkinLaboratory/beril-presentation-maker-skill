...
| D — Adam reads both decks + subjective scoring (metric 5) + veto decision (D-066) | review + DECISION | ✅ committed 2026-05-25 (Adam read all 4 decks: ibd v0.3 draft_4 + v0.4 draft_5 both equally poor on arc (Likert 2/5 each); fdm v0.3 draft_2 significantly better than v0.4 draft_5 (Likert 3 vs 2; v0.4 regressed via cross-slide consistency loss). Adam-veto per D-066 = **don't ship**; v0.4 stays opt-in via `--architecture-pipeline v0_4`. Three drivers: v0.4 doesn't help on ibd narrative; v0.4 actively regresses on fdm (10/137 vs 10/151 cross-slide error + substory_arc breakage + missing_slide P0); upstream content-shape weakness in BOTH pipelines (obscure arc, walls of text, specialist register leakage, no unifying point) — opens v0.5 content-discipline milestone. Mechanical FAIL too (v0.4 wins 2/6 on target, needs ≥4; max wall-clock reduction -36.7%, needs ≥40%). Outcome lands as D-069 + opens D-070) |
| E — SPEC + DECISIONS + LAYOUT + RELEASE_NOTES (cut-over outcome) | docs | ✅ committed 2026-05-25 (V0_4_ARCHITECTURE §16 M6 → SHIPPED block with 8-tier ledger + per-tier commit refs + carry-out items; DECISIONS appends D-068 (Tier C.1 caption demote retroactive entry), D-069 (Tier-D veto outcome), D-070 (v0.5 content-discipline scope opening); LAYOUT.md adds `m6_score.py` + summary list update; RELEASE_NOTES adds v0.4.0-experimental entry with operational guidance (when to use v0.4 / when not to); SPEC.md unchanged (M6 outcome is V0_4_ARCHITECTURE + RELEASE_NOTES territory, not per-slide-contract)) |
| F — closeout (V0_4_ARCHITECTURE §16 M6 → SHIPPED; auto-memory; v0.4.0 tag if pass) | paperwork + tag | ✅ committed 2026-05-25 (M6_PUNCH_LIST status table closed; auto-memory `project_presentation_maker_v0_4_m6.md` written (M6 retrospective + lessons + 5 cross-cutting findings); MEMORY.md index promoted M6 to top of Active work; V0_5_PUNCH_LIST.md drafted (content-discipline milestone per D-070); git tag `v0.4.0-experimental` (NOT v0.4.0 — opt-in status explicit in tag)) |
