...
| D — SPEC.md §13.1 docs update + DECISIONS update | `SPEC.md`, `DECISIONS.md` D-059..D-061 | ✅ committed 2026-05-24 (SPEC §13.1 updated with v0.4 P3 mechanism + severity mapping per D-061 anti-double-lift split; §13.2 dispatch table row updated; DECISIONS D-059 (P3 retirement closes D-058), D-060 (DQ1 heuristic + DQ2 per-slide hedge agg), D-061 (DQ3 hard-reject + DQ4 split); suite 1212 passed) |
| E — end-to-end smoke on `ibd_phage_targeting` | live (E1 cascade re-run) + synthetic CLI (E2) | ✅ verified 2026-05-24 (E1: new P3 + cascade on `ibd_phage_targeting/draft_1` — Tier 1 fail, 306 findings (was 586 with v0.3 P3 — 282 false-positives gone), P0=2 (two real high-severity ratios: 45/51 on slide 8, 26/188 on slide 24), `short_circuited_at: tier1` ✓ fail-fast restored. E2: revise_invariance CLI on synthetic pre/post slides — case 1 citation-deletion fires `verdict=fail, rc=1`, [Smith2024] flagged in violations; case 2 clean prose-tightening passes `verdict=pass, rc=0` with 4 invariants checked + claim_id skipped (no inventory). Spec-not-mutated guaranteed structurally by Tier B gate placement, pinned by `test_loop_state_tracks_invariance_violations_separately`) |
| F — closeout | paperwork | ✅ committed 2026-05-24 (V0_4_ARCHITECTURE §16 M5 → M5a SHIPPED + M5b deferred; LAYOUT.md adds `revise_invariance.py` + updates `review_cascade.py` D-058→D-059 note; auto-memory `project_presentation_maker_v0_4_m5a.md` + MEMORY.md index entry; status table closed) |
