...
| F — closeout | paperwork | ✅ committed 2026-05-24 (V0_4_ARCHITECTURE §16 M4b → SHIPPED with the 6-tier ship table + per-tier commit ledger; LAYOUT.md has `review_cascade.py` + `review_tier2.py` + `review_tier2.v1.md` in §1 tree + "Runs at draft time" note; DECISIONS.md D-054..D-057 (DQ1–DQ4) added alongside the pre-existing D-058 P3 demote; README + HUB_INSTALL prominently document `--visual-qa` per Adam's DQ2 emphasis + new "Review cascade" subsections; auto-memory `project_presentation_maker_v0_4_m4b.md` + MEMORY.md index line) |
