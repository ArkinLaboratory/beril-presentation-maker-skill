...
| A.1 — `imageConfig` wrapper fix (live-discovery patch) | `tools/image_client.py::_call_google_ai_studio` + tests | ✅ ready to commit 2026-05-24 (E2 live surfaced 400 INVALID_ARGUMENT — Tier-A used `generationConfig.responseFormat.image.*` per ai.google.dev human-docs page, but v1beta `$discovery/rest` schema reveals the correct wrapper is `generationConfig.imageConfig.*` (`responseFormat` takes a different schema, `ResponseFormatConfig`); 3 lines of body fix + docstring update; regression test `test_ai_studio_request_shape_does_not_use_response_format_wrapper` pins the absence of the wrong wrapper so a future refactor can't reintroduce it; suite 1279 passed) |
| B — orchestrator auth discovery (mirror CBORG block for AI Studio) + `--image-provider` flag wiring | `tools/presentation_maker.sh` edit | ✅ ready to commit 2026-05-24 (per DQ1/D-062: existing CBORG `.env` parse extended into a single-pass loop resolving BOTH `CBORG_API_KEY` + `GOOGLE_AI_STUDIO_API_KEY`; new `IMAGE_PROVIDER` precedence block resolves the active provider — `--image-provider` arg > AI Studio env > CBORG env > empty; `--image-provider` CLI flag wired with help-block docs; explicit unknown provider exits 2; explicit known provider with missing env var prints WARNING; `image_client.py generate` invocation at line 1809 gains `--provider "$IMAGE_PROVIDER"` arg; 13 new orchestrator-snippet tests exercising the auth block via synthetic `.env` + `bash -c` extraction harness (precedence, .env loading, quoted values, missing-file graceful, env-wins-over-dotenv, help-text discoverability); suite 1249 passed) |
| C — model-availability probe + sidecar cache (`audit/ai_image_gen_probe.json`) + DQ3 fallback chain | `tools/image_client.py` (new helper) + orchestrator wire | ✅ ready to commit 2026-05-24 (5 new module-level helpers — `probe_available_models` GETs `/v1beta/models` + filters image-capable; `resolve_ai_studio_model` walks D-035-rev1 chain; `_fingerprint_api_key` for non-reversible cache identity; `load_or_probe_ai_studio_model` round-trips the sidecar at `audit/ai_image_gen_probe.json` per D-063 with override/force-refresh/corrupt-recovery; `format_probe_failure_diagnostic` builds the D-064 multi-line stderr message; new CLI `probe` subcommand emits resolved model on stdout / rc=5 + diagnostic on no-match; orchestrator stage_image_gen runs probe once + branches per D-064 hybrid: rc=0→use resolved model, rc=5+CBORG→silent fallback, rc=5+no-CBORG→loud-warning disable; `GOOGLE_AI_STUDIO_RESOLVED_MODEL` env threaded into generate invocation; `GOOGLE_AI_STUDIO_MODEL` env override short-circuits probe per C5; 29 new tests; suite 1278 passed) |
| D — SPEC.md §8.3 + DECISIONS.md D-062..D-064 + LAYOUT.md | docs | ✅ ready to commit 2026-05-24 (SPEC §8.3 rewritten for two-provider posture + new §8.3.2 narrative on probe + cache + D-064 hybrid fallback; LAYOUT.md `image_client.py` short entry expanded with M5b notes + §14 long entry rewritten for AI Studio + probe + D-064; DECISIONS appends D-035-rev1 (Google's May-2026 model line shifted under D-035; chain updated; rev preserves cross-refs), D-062 (auth-discovery in shell orchestrator), D-063 (sidecar cache + fingerprint), D-064 (hybrid fallback); V0_4_ARCHITECTURE §14.2 probe section updated to point at D-035-rev1 + D-063 sidecar + D-064 hybrid + C5 override env var; suite 1278 still passing) |
| E — Tier-E live smoke: 1 image per path + AI Studio calibration re-run (`image_gen_calibration.py` against AI Studio) | live (~$0.55) | ✅ verified 2026-05-24 (E1 probe rc=0 + sidecar cache verified; E2 AI Studio: production CLI rendered 1024² JPEG via `gemini-3.1-flash-image-preview` ($0.045, 25s); E2 CBORG: documented out-of-scope failure (CBORG tenant has 0 image-gen-capable models); E3 calibration: 13 trials, 8 ok / 5 timeouts at 300s — cost mean $0.0463 σ=$0.0013 max $0.0485 cumulative $0.3705; two calibration-driven updates: `_WORST_CASE_COST_USD` 0.05 → 0.08 (1.65× margin over observed max; was 1.03×), default `timeout_s` 120 → 360 (1.66× margin over observed 217s max); calibration artifact at `audit/image_gen_calibration_ai_studio_2026-05-24.md`; suite 1279 passed) |
| F — closeout (V0_4_ARCHITECTURE §16 M5b → SHIPPED; auto-memory) | paperwork | ✅ committed 2026-05-24 (V0_4_ARCHITECTURE §16 M5b → SHIPPED with full 7-tier ship block (A/A.1/B/C/D/E/F) mirroring M5a/M4b format; per-tier commit refs + carry-out items; auto-memory `project_presentation_maker_v0_4_m5b.md` (cross-cutting lessons: discovery-doc-beats-human-docs, mocked-tests-aren't-contract-tests, calibration-drives-constants, bimodal-wall-clock, API-key-tier-confusion); MEMORY.md index promoted M5b to top of Active work; status table closed) |
