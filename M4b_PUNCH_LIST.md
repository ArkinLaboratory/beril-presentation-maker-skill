...
| E — end-to-end cascade smoke on `ibd_phage_targeting` | live | 🔄 in flight: offline half committed (4 integration tests in `tests/integration/test_review_cascade_e2e.py` — synthetic P3 short-circuit + clean-spec all-tiers + orchestrator de-dup contract; suite 1169 passed). Live calibration probe (`.m4b-tier-e-runbook.md`) pending operator run on `ibd_phage_targeting/draft_1`. |
