...
| E — end-to-end cascade smoke on `ibd_phage_targeting` | live | 🔄 in flight: (a) offline half committed (4 integration tests; synthetic P3 short-circuit + clean-spec all-tiers + orchestrator de-dup contract). (b) live cascade probe ran on `ibd_phage_targeting/draft_1` (2026-05-24); revealed P3 numeric-provenance fires on 282 numbers (v0.3 `speaker_notes_provenance` contract is dead on v0.4) — short-circuited every v0.4 draft. Fixed: D-058 demotes P3 from P0 to P1 on the v0.4 cascade; M5 schedules retire-or-rewrite-to-wrap-`check_quantitative_grounding`. Suite 1169 passed. Live re-probe (Tier 2 Haiku + Tier 3 adversarial) pending operator re-run; calibration capture pending. |
