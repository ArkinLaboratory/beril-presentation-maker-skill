...
| E — end-to-end cascade smoke on `ibd_phage_targeting` | live | 🔄 in flight: (a) offline half committed (4 integration tests; synthetic P3 short-circuit + clean-spec all-tiers + orchestrator de-dup contract). (b) live cascade round 1 (2026-05-24) revealed P3 v0.3-era impedance mismatch → D-058 demotes P3 to P1 on v0.4 cascade; M5 schedules retirement. (c) live cascade round 2 (2026-05-24): Tier 1 advisory (586 findings, no P0), Tier 2 ran clean (11 findings across all 4 detection classes — 113.7s on Haiku), Tier 3 errored on `--beril-root` resolution (standalone cascade invocation didn't have BERIL_ROOT env that orchestrator preamble sets). Fixed: `_invoke_beril_adversarial` now resolves BERIL_ROOT via explicit arg → env var → walk-up-4-parents fallback, passes `--beril-root` explicitly. Suite 1174 passed. Round-3 re-probe pending. |
