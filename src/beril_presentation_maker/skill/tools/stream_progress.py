#!/usr/bin/env python3
"""Parse `claude -p --output-format stream-json` output for the
beril-presentation-maker orchestrator.

Forked from beril-adversarial v0.1.x with no functional changes — same
two purposes (Write-tool verification + cost summary) and identical exit
codes. The fork exists so each skill can iterate independently; if/when
we factor out a shared dependency (`beril-skill-common`), this file can
collapse back into it.

Two purposes, both load-bearing:

1. **Programmatic Write-tool verification.** Detects the silent-failure
   case where claude produces a chat-response without invoking Write.
   Exit code 2 lets the orchestrator's retry helper kick in.
...
def _write_metadata_json(metadata_path: Path, metadata: dict) -> bool:
    ...
    Used by the orchestrator (presentation_maker.sh, lands in
    v0.1.0-prompts) to compute cumulative cost across multiple claude
    calls in the drafting pipeline (plan + throughline + substory +
    slide_compose × N + speaker_notes + qa_prep + reframer + rewrite).
    The aggregator at end of pipeline sums these and writes ONE
    cumulative Run Metadata section into audit/cost-summary.md.
...
def parse_stream(
    ...
    # Success — write per-call metadata JSON for the orchestrator-level
    # aggregator to combine across calls in the drafting pipeline.
    if metadata_out:
        meta = {
...
def main() -> int:
    ...
    p.add_argument("--metadata-out", default=None,
                   help="On success, write per-call metadata JSON to this path. "
                        "The orchestrator aggregates these across calls and "
                        "writes one cumulative Run Metadata section.")
    p.add_argument("--label", default="presentation-maker",
                   help="Label used in the end-of-run summary line.")
    args = p.parse_args()
