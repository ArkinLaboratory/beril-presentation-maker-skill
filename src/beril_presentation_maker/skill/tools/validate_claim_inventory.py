...
Vendored into beril-presentation-maker v0.4 per D-040-rev1 (2026-05-12)
from beril-paper-writer. Re-vendored 2026-05-21 to paper-writer v1.0.0
(commit 46e8943): picks up the Stage 3 Tier I notebook-repair pass
(validator 0.2.0-stage3-tierI). Source-of-truth upstream: spike/
beril-paper-writer-skill-draft/src/beril_paper_writer/skill/tools/
validate_claim_inventory.py. Module body is byte-portable; this copy
adds only this attribution. Presentation-maker invokes this validator
from `tools/extract_claims.py` (the M1 adapter) after `claude -p`
produces the LLM-extracted TSV.

Stage 1 Tier C of paper-writer's STAGED_IMPROVEMENT_PLAN.md. The
orchestrator's phase_triage produces claim_inventory.tsv via an LLM
prompt (extract_claims.v1.md). The LLM emits source_notebook paths that
