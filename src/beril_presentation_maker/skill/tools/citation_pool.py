...
Forked from beril-paper-writer-skill/citation_pool.py 2026-04-26 with
two presentation-maker-specific additions:

  - Output filename is `citation_pool.json` (LAYOUT.md output tree),
    matching the slide_spec.json `citation_pool_ref` convention. Paper-
    writer uses `pool.json`; load_from_disk handles both names so a
    paper-writer pool can be loaded directly without rename.

  - `--reuse-from-paper PATH` CLI option (and `merge_pools()` helper):
    if a sibling paper-writer draft has been built, its citation pool
    can be loaded as starting state for the presentation\'s pool. Per
    SPEC §9.2: talks usually cite background that papers don\'t, so the
    pool is a starting set, not a frozen final.

The body — 10-field strict citation discipline, 80-cap, DOI/PMID
dedup, references.md / bibliography.bib / citation_map.md
serialization, validate_entry / validate_pool / assign_bib_keys —
is unchanged from paper-writer.

Standalone CLI + importable module, mirroring the other
extractors in this skill.
"""
...
def merge_pools(target: CitationPool, source: CitationPool) -> dict:
    """Merge entries from `source` into `target`. Entries are matched
    by DOI (preferred) or PMID; if neither matches an existing target
    entry, the source entry is appended (subject to POOL_SIZE_CAP).

    Returns a summary dict: {added, skipped_duplicate, skipped_full}.

    Per SPEC §9.2: paper-writer pool reuse should be additive — talks
    pick up paper background but may need extra citations. This merge
    is purely structural; the slide-compose prompt is responsible for
    adding new entries beyond reuse.
    """
    summary = {"added": 0, "skipped_duplicate": 0, "skipped_full": 0}
    target_dois = {e.doi.lower() for e in target.entries if e.doi}
    target_pmids = {e.pmid for e in target.entries if e.pmid}

    for entry in source.entries:
        if entry.doi and entry.doi.lower() in target_dois:
            summary["skipped_duplicate"] += 1
            continue
        if entry.pmid and entry.pmid in target_pmids:
            summary["skipped_duplicate"] += 1
            continue
        if len(target.entries) >= POOL_SIZE_CAP:
            summary["skipped_full"] += 1
            continue
        target.entries.append(entry)
        if entry.doi:
            target_dois.add(entry.doi.lower())
        if entry.pmid:
            target_pmids.add(entry.pmid)
        summary["added"] += 1
    return summary


def load_pool_from_paper_draft(paper_draft_dir: Path) -> CitationPool:
    """Convenience wrapper: load a paper-writer draft's citation pool.

    Equivalent to `load_from_disk(paper_draft_dir)`, kept as a separate
    name so the call site reads naturally:

        paper_pool = load_pool_from_paper_draft(Path("papers/draft_1"))
        merge_pools(my_pool, paper_pool)
    """
    return load_from_disk(paper_draft_dir)
...
def _cmd_load(args: argparse.Namespace) -> int:
    """Read citation_pool.json from a draft dir and emit it as JSON to stdout."""
    pool = load_from_disk(args.draft_dir)
    sys.stdout.write(json.dumps(pool.to_dict(), indent=2) + "\n")
    return 0


def _cmd_reuse_from_paper(args: argparse.Namespace) -> int:
    """Load a paper-writer draft's pool and merge into a presentation
    draft's pool. Per SPEC §9.2."""
    paper_dir = Path(args.paper_draft_dir)
    talk_dir = Path(args.talk_draft_dir)
    if not paper_dir.is_dir():
        print(f"paper draft dir not found: {paper_dir}", file=sys.stderr)
        return 2
    paper_pool = load_pool_from_paper_draft(paper_dir)
    if not paper_pool.entries:
        if not args.quiet:
            print(f"[citation_pool] paper draft has no pool to reuse from "
                  f"({paper_dir}); nothing to do", file=sys.stderr)
        return 0
    talk_pool = load_from_disk(talk_dir) if talk_dir.is_dir() else CitationPool()
    summary = merge_pools(talk_pool, paper_pool)
    talk_dir.mkdir(parents=True, exist_ok=True)
    paths = serialize_to_disk(talk_pool, talk_dir)
    if not args.quiet:
        print(f"[citation_pool] reused {summary['added']} entries from "
              f"{paper_dir.name} (skipped {summary['skipped_duplicate']} "
              f"duplicates, {summary['skipped_full']} pool-full)",
              file=sys.stderr)
        print(f"[citation_pool] wrote {paths['citation_pool.json']}",
              file=sys.stderr)
    return 0
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p_fmt = sub.add_parser(
        "format",
        help="Read citation_pool.json and write references.md / bibliography.bib / citation_map.md.",
    )
    p_fmt.add_argument("pool_json", type=Path,
                       help="Source citation_pool.json (or paper-writer's pool.json)")
    p_fmt.add_argument(
        "draft_dir", type=Path,
        help="Target draft directory (talks/draft_N/).",
    )
    p_fmt.set_defaults(func=_cmd_format)

    p_load = sub.add_parser(
        "load",
        help="Load a draft directory's citation_pool.json and emit it as JSON to stdout.",
    )
    p_load.add_argument(
        "draft_dir", type=Path,
        help="Draft directory to read citation_pool.json from "
             "(falls back to pool.json for paper-writer compat).",
    )
    p_load.set_defaults(func=_cmd_load)

    p_reuse = sub.add_parser(
        "reuse-from-paper",
        help="Load a paper-writer draft's pool and merge into a presentation "
             "draft's pool. Per SPEC §9.2 reuse pattern.",
    )
    p_reuse.add_argument(
        "paper_draft_dir", type=Path,
        help="Paper-writer draft dir (papers/draft_N/) — source pool.",
    )
    p_reuse.add_argument(
        "talk_draft_dir", type=Path,
        help="Presentation-maker draft dir (talks/draft_N/) — target pool. "
             "If citation_pool.json doesn't exist there yet, it'll be "
             "created from the paper's pool.",
    )
    p_reuse.add_argument(
        "--quiet", action="store_true",
        help="Suppress merge summary on stderr",
    )
    p_reuse.set_defaults(func=_cmd_reuse_from_paper)
