...
Vendored from beril_paper_writer.discovery with the SKILL_DIR_NAME
changed to 'beril-presentation-maker'. The skills coexist in the same
...
Derived paths are all relative to BERIL_ROOT:
  - .env             → `<root>/.env`
  - skill dir        → `<root>/.claude/skills/beril-presentation-maker/`
  - prompts          → `<root>/.claude/skills/beril-presentation-maker/prompts/`
  - references       → `<root>/.claude/skills/beril-presentation-maker/references/`
  - tools            → `<root>/.claude/skills/beril-presentation-maker/tools/`
  - state            → `<root>/.claude/skills/beril-presentation-maker/state/`
  - projects         → `<root>/projects/`
  - talks            → `<root>/projects/<project_id>/talks/draft_N/`
"""
...
SKILL_DIR_NAME = "beril-presentation-maker"
...
def find_beril_root(
    ...
    diag_lines = [
        ...
        "  - Run beril-presentation-maker from inside a BERIL checkout.",
        "",
    ...
    diag_lines.extend([
        ...
        "https://github.com/ArkinLaboratory/beril-presentation-maker-skill/issues.",
    ])
    raise BerilRootNotFound("\n".join(diag_lines))
...
def get_talks_dir(beril_root: Path, project_id: str) -> Path:
    """`<beril_root>/projects/<project_id>/talks/` — where draft_N/ dirs live."""
    return get_project_dir(beril_root, project_id) / "talks"


def get_draft_dir(beril_root: Path, project_id: str, draft_number: int) -> Path:
    """`<beril_root>/projects/<project_id>/talks/draft_N/`."""
    return get_talks_dir(beril_root, project_id) / f"draft_{draft_number}"
