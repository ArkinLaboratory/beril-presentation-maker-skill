"""`beril-presentation-maker template-env` — print the CRAFT `.env` config block.
...
has two parts:
  ...
  - A **shared CRAFT block** (provider + model tiers + marker; NO
    credentials per the additive-only contract) that is written ONCE per
    BERIL deployment and shared by every CRAFT skill. `configure`
    detects the `# >>> CRAFT shared config` sentinel and does NOT
    duplicate it if another skill already wrote it. The SHARED_BLOCK
    text below is BYTE-IDENTICAL to the adversarial-skill copy — the
    canary exemplar — so any CRAFT skill can append it first without
    drift.
  - A **per-skill marker** (`BERIL_PRESENTATION_MAKER_CONFIGURED_*`)
    that each skill stamps independently on a successful configure.
...
(+ gitignored `settings.local.json` for the token) FROM the resolver
output, so that `claude -p` picks up provider routing and the model-tier
aliases natively. Do not put `ANTHROPIC_BASE_URL` / `ANTHROPIC_AUTH_TOKEN`
here — `claude -p` does not read `.env`; those are generated into
settings.json.
"""
...
# Byte-identical to the canary adversarial exemplar (37088d8); divergence
# here would break the cross-skill conformance fixture (Stage 6).
SHARED_BLOCK = """\
...
def _per_skill_block() -> str:
    return f"""\

# --- beril-presentation-maker-skill (per-skill) ---
# Image generation is OPTIONAL. If GOOGLE_AI_STUDIO_API_KEY is set, the
# Google AI Studio path is preferred. Else if CBORG_API_KEY is set, the
# CBORG image path is used. Neither set → image-gen disables itself
# (curated figures only); the deck draft continues. See
# tools/image_client.py + tools/image_gen_orchestrate.py.

# Written by `beril-presentation-maker configure` on a successful smoke.
# Do not edit by hand; re-run configure to refresh.
BERIL_PRESENTATION_MAKER_CONFIGURED_AT=
BERIL_PRESENTATION_MAKER_CONFIGURED_VERSION=
# beril-presentation-maker-skill v{__version__}
"""
