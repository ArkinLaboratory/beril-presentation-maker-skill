"""`beril-presentation-maker configure` — CRAFT runtime-config bootstrapper.

Implements the configure half of CRAFT-CONTRACT §3.4 (Runtime configuration
contract v2) for the presentation-maker — copied from the adversarial canary (CRAFT-CONTRACT §3.4):
  ...
  6. Run a validation ping that asserts the response token (`reply: ok`
     ...
     detect that.
  7. On success, stamp BERIL_PRESENTATION_MAKER_CONFIGURED_AT / _VERSION into
     `.env`.
...
from beril_presentation_maker import __version__, discovery, llm_config
from beril_presentation_maker.commands import template_env
...
PER_SKILL_MARKER = "# --- beril-presentation-maker-skill (per-skill) ---"
...
def run(args: argparse.Namespace) -> int:
    ...
    print(f"beril-presentation-maker-skill v{__version__}")
    ...
    stamped = update_or_append_kv(env_text, "BERIL_PRESENTATION_MAKER_CONFIGURED_AT", now)
    stamped = update_or_append_kv(
        stamped, "BERIL_PRESENTATION_MAKER_CONFIGURED_VERSION", __version__
    )
    if stamped != env_text:
        env_path.write_text(stamped)
        print(f"  [OK] stamped BERIL_PRESENTATION_MAKER_CONFIGURED_AT={now}")

    # Image-gen provider availability is informational (CRAFT-CONTRACT
    # §3.4: image generation is independent + optional with graceful
    # degrade). Both keys read from the BERIL .env (additive-only: we
    # don't re-declare them).
    google_key = env_map.get("GOOGLE_AI_STUDIO_API_KEY", "").strip()
    cborg_key = env_map.get("CBORG_API_KEY", "").strip()
    if google_key:
        print("  [OK] image-gen: GOOGLE_AI_STUDIO_API_KEY set → Google AI Studio path")
    elif cborg_key:
        print("  [OK] image-gen: CBORG_API_KEY set → CBORG image path (OpenAI-style /v1)")
    else:
        print(
            "  [info] no image-gen key (GOOGLE_AI_STUDIO_API_KEY / CBORG_API_KEY); "
            "deck draft will degrade gracefully (curated figures only)"
        )
