...
- F1. **CLOSED — wired in M3 Tier A, 2026-05-23** (`stage_phase0_tooling` + the v0.4 dispatch re-sequencing; see `M3_PUNCH_LIST.md` Tier A). The orchestrator now invokes `phase0_reuse.py` at the `phase0_tooling` phase. Original note: it should be invoked at the new `phase0_tooling` phase per V0_4_ARCHITECTURE §10.1; the `PHASE0_DIR` / `METHODS_PROVENANCE_PHASE0` / `CLAIM_INVENTORY_PHASE0` vars were ultimately defined in the orchestrator's own `set_draft_paths()` (M3 Tier A A4) — `presentation_maker.sh` hand-rolls its layout vars rather than sourcing `draft_paths.shell_exports()`.
